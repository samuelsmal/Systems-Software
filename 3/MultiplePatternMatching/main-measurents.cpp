...
int main(int argc, char* argv[])
{
	...
	std::chrono::duration<double> current_best_time =  std::chrono::system_clock::duration::max();
	int best_number_of_threads = 0;
	
	for(number_of_threads = 4; number_of_threads < 40; ++number_of_threads)
	{
		start = std::chrono::system_clock::now();
	
		for (u_long counter = 0; counter < 100; ++counter)
		{
		...
		if (elapsed_seconds < current_best_time) 
		{
			current_best_time = elapsed_seconds;
			best_number_of_threads = number_of_threads;
		}
		std::cout << "current threads: " << number_of_threads << std::endl
			<< "current time: " << elapsed_seconds.count() << std::end;
    ...
    std::cout << "number of threads " << best_number_of_threads
              << "best elapsed time: " << current_best_time.count() << "s" << std::endl;
